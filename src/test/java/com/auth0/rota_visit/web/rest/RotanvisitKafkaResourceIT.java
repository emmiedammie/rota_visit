package com.auth0.rota_visit.web.rest;
...
import com.auth0.rota_visit.IntegrationTest;
import com.auth0.rota_visit.config.EmbeddedKafka;
import com.auth0.rota_visit.config.KafkaSseConsumer;
import com.auth0.rota_visit.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class RotanvisitKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/rotanvisit-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/rotanvisit-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/rotanvisit-kafka/unregister"));
                return;
            }
        }
